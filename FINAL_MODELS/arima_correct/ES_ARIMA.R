...
#  !!!!!!!!!!!!!!!!!!
#  ! iter starts in 2 to get the same results as the ARIMAX  
#  !!!!!!!!!!!!!!!!!!
        ...
        tryCatch(
          expr = {
            ...
            if(!auto){ 
              ...
              fit<-arima(ts(log1p(current_state_tb$cases[rolling_window]), deltat = 1/52), order = order_params[j,], method = "CSS-ML") 
              }
            # if auto = TRUE, run auto.arima
            else{
              # fit an auto.arima on the log1p of the data
              fit<-invisible(auto.arima(ts(log1p(current_state_tb$cases[rolling_window]), deltat = 1/52) ,stepwise=TRUE,approximation=FALSE, 
                                        allowdrift=FALSE,

...
ROLLING_ARIMAX_TEMP <- function(current_state_tb, week_lag=week_lag, temperature_data=NULL, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE) {
  ...
  for(iter in (1+week_lag):(NROW(current_state_tb$cases)-(window))){ # rolling window of 104 weeks up to the end of the ILI cases dataset
    ...
    # current_state_rolling_window e.g. [2:105]->[3:106]->[4:107]
    current_state_rolling_window<- (iter):(window+iter-1)  
    # exog_rolling_window e.g. [1:104]->[2:105]->[3:106], for 'lag_week=1'
    exog_rolling_window<- (iter-week_lag):(window+iter-1-week_lag)  

    # ROLLING EXOGENOUS DATA
    # we use the exog_rolling_window on the temperature data for the current state for fitting it with a lag
    temp_rolling_window<-data.frame(temperature_data[,current_state_tb$location[1]][exog_rolling_window]) 
    
    # EXOGENOUS VARIABLE UTILIZED IN THE FORECAST 
    # We need an exogenous variable from the date we are doing our forecast
    # We use the last date of the current_state_rolling_window to get this variable from the temperature data
    # Note that this exogenous variable was not utilized during the fitting process.
    ...
    # Fitting 
    ##########
    fitted_models<-list() # List that will get each fitted ARIMA
    model_aic_scores<-c() # AIC scores for weighting the ensembles
    model_id<-1 # Each model id, use later on a for loop
    
    # Start if the current_state_rolling_window has 104 elements
    if(length(current_state_tb$cases[current_state_rolling_window])==window){ 
        ...
        tryCatch(
          expr = {
            ...
            # if auto = TRUE, run auto.arima
              ...
              # use the temperature data on based on a lag rolling window as exogenous variable 
              fit<-invisible(auto.arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=temp_rolling_window[,1] ,stepwise=TRUE,approximation=FALSE,
      ...
      for(my_model in fitted_models){
        ...
        if(length(my_model)>0 && !is.na(my_model[1]) && !(is.na(my_model$aic))){ 
          ...
          ####################################################################################
          # Calculate the upper and lower bounds of the prediction intervals for 23 quantiles

...
ROLLING_ARIMAX_EPIWEEK <- function(current_state_tb, n_weeks_ahead=1, window = 104, order_params=NULL, auto=FALSE, week_lag=week_lag) {
  ...
  for(iter in  (1+week_lag):(NROW(current_state_tb$cases)-(window))){ # rolling window of 104 weeks up to the end of the ILI cases dataset
    ...
    # we use the exog_rolling_window on the current_state data, which has a lag 
    ...
    # we split the data in [1:52] and [53:104] weeks, then we sum and divide by 2
    # this gives of the mean cases by epidemiological weeks for the last 2 years
        ...
        tryCatch(
          expr = {
            ...
            # if auto = TRUE, run auto.arima
              ...
              # use the epiweek data on based on a lag rolling window as exogenous variable 
              fit<-invisible(auto.arima(log1p(current_state_tb$cases[current_state_rolling_window]), xreg=log1p(epiweek_dataset[,1]) ,stepwise=TRUE,approximation=FALSE,
